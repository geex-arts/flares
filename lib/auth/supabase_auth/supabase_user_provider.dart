...
class FlaresSupabaseUser extends BaseAuthUser {
  FlaresSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> flaresSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FlaresSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
